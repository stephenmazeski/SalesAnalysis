...
#ggplot(data=MergedSplit,aes(x=OrderDayoftheWeek,y=AvgDayShip))+geom_point(size = 3)+aes(colour=Product, shape = AddressType)#+facet_wrap(~Product,ncol=4) 
ggplot(data=MergedSplit2,aes(x=OrderDayoftheWeek,y=AvgDayShip))+geom_point(size = 3)+aes(colour=Product, shape = AddressType)+facet_wrap(~Zone,ncol=4) + ggtitle("Zone Impacts on How Long it Takes to Ship")
